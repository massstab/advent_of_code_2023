...
def farthest_node(graph, start_node):
    # Use BFS to calculate distances from the start node
    distances = nx.single_source_shortest_path_length(graph, start_node)

    # Find the node with the maximum distance
    farthest_node, max_distance = max(distances.items(), key=lambda x: x[1])
...
def visualize_graph(graph, grid_size, symbols, start_node):
    ...
    node_color = ['red' if node == start_node else 'skyblue' for node in graph.nodes]

    # Draw the graph using the grid layout
    nx.draw(graph, pos=pos, with_labels=True, labels=labels, font_weight='bold', node_size=140,
            node_color=node_color, font_color='black', font_size=12, edge_color='gray', width=4)
...
farthest, distance = farthest_node(
    G, start_node)

print(f"The farthest node from node {start_node} is {farthest} with a distance of {distance}.")

visualize_graph(G, gridsize, symbols=dict(enumerate(flatten)), start_node=start_node_index)
